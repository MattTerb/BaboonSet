...
from .utils import create_board_object_pts, save_points, load_points, \
    ...
    find_scene_file, save_optimised_baboon, save_3d_baboon_as_2d
...
def plot_scene(data_dir, scene_fname=None, manual_points_only=False, **kwargs):
    *_, scene_fpath = find_scene_file(data_dir, scene_fname, verbose=False)
    ...
    if manual_points_only:
        ...
        points_fpaths = sorted(glob(os.path.join(points_dir, 'points[1-2].json')))
...
def plot_baboon_states(states, smoothed_states=None, out_fpath=None, mplstyle_fpath=None):
...
def _plot_baboon_reconstruction(positions, data_dir, scene_fname=None, labels=None, **kwargs):
    positions = np.array(positions)
    *_, scene_fpath = find_scene_file(data_dir, scene_fname, verbose=False)
    ca = Baboon(positions, scene_fpath, labels, project_points_fisheye, **kwargs)
    ca.animation()
    
    
def plot_baboon_reconstruction(data_fpath, scene_fname=None, **kwargs):
    ...
    _plot_baboon_reconstruction([positions], os.path.dirname(data_fpath), scene_fname, labels=[label], **kwargs)
    

def plot_multiple_baboon_reconstructions(data_fpaths, scene_fname=None, **kwargs):
    positions = []
    labels = []
    for data_fpath in data_fpaths:
        ...
        labels.append(label)
    _plot_baboon_reconstruction(positions, os.path.dirname(data_fpath), scene_fname, labels, **kwargs)
...
def save_tri(positions, out_dir, scene_fpath, start_frame, dlc_thresh, save_videos=True):
    ...
    save_optimised_baboon(positions, out_fpath, extra_data=dict(start_frame=start_frame))
    save_3d_baboon_as_2d(positions, out_dir, scene_fpath, get_markers(), project_points_fisheye, start_frame)
...
def save_ekf(states, out_dir, scene_fpath, start_frame, dlc_thresh, save_videos=True):
    ...
    save_optimised_baboon(positions, out_fpath, extra_data=dict(smoothed_positions=smoothed_positions, **states, start_frame=start_frame))
    save_3d_baboon_as_2d(smoothed_positions, out_dir, scene_fpath, get_markers(), project_points_fisheye, start_frame)
...
def save_fte(states, out_dir, scene_fpath, start_frame, dlc_thresh, save_videos=True):
    ...
    save_optimised_baboon(positions, out_fpath, extra_data=dict(**states, start_frame=start_frame))
    save_3d_baboon_as_2d(positions, out_dir, scene_fpath, get_markers(), project_points_fisheye, start_frame)
