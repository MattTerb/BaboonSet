...
def load_dlc_points_as_df(dlc_df_fpaths, verbose=True):
    ...
    for path in dlc_df_fpaths:
        ...
        Index = Dataframe[scorer][baboonNum]
        dlc_df = Index
        dlc_df = dlc_df.swaplevel(0,1,axis=1).T.unstack().T.reset_index().rename({'level_0':'frame'}, axis=1)
        dlc_df.columns.name = ''
        dfs.append(dlc_df)
        baboonNum = 'baboon3'
...
def load_rotation(csv_path):

    rotationData = pd.read_csv(csv_path, sep=';')
...
def save_optimised_baboon(positions, out_fpath, extra_data=None, for_matlab=True, save_as_csv=True):
    ...
    if save_as_csv:
        # to-do??
        # should use a similar method as save_3d_baboon_as 3d, along the lines of:
...
def save_3d_baboon_as_2d(positions_3d, out_dir, scene_fpath, bodyparts, project_func, start_frame, save_as_csv=True, out_fname=None):
    ...
    video_fpaths = sorted(glob(os.path.join(out_dir, 'cam[1-2].mp4'))) # check current dir for videos
    if not video_fpaths:
        video_fpaths = sorted(glob(os.path.join(os.path.dirname(out_dir), 'cam[1-2].mp4'))) # check parent dir for videos
    ...
    if video_fpaths:
        ...
        print('Could not save 3D baboon to 2D - No videos were found in', out_dir, 'or', os.path.dirname(out_dir))

# ========== OTHER ==========

def find_scene_file(dir_path, scene_fname=None, verbose=True):
    if scene_fname is None:
        n_cams = len(glob(os.path.join(dir_path, 'cam[1-2].mp4'))) # reads up to cam2.mp4 only
        scene_fname = f'{n_cams}_cam_scene_sba.json' if n_cams else '[1-2]_cam_scene*.json'

    if dir_path and dir_path != os.path.join('..', 'data'):
        scene_fpath = os.path.join(dir_path, 'extrinsic_calib', scene_fname)
        # ignore [1-2]_cam_scene_before_corrections.json unless specified
        ...
        if scene_files:
            ...
            n_cams = int(scene_fname[0]) # assuming scene_fname is of the form '[1-2]_cam_scene*'
